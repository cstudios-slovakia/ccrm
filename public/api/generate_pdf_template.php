...
$systemLanguage = in_array(($data['systemLanguage'] ?? 'sk'), ['sk', 'hu', 'en'], true)
    ? $data['systemLanguage']
    : 'sk';
...
Write every human-readable value (name, description, customBannerText) in this language code: {$systemLanguage}.
...
curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode([
    // Admin-configured model rather than a literal, like every other AI endpoint.
    'model' => ccrm_ai_model($integrationsConfig),
    'messages' => [
...
if (!is_array($sectionsOrder)) {
    $sectionsOrder = $requiredSections;
}
// Only known section ids survive — the renderer has no notion of any other.
$sectionsOrder = array_values(array_intersect($sectionsOrder, $requiredSections));
...
/**
 * The blueprint is model output, so every field is untrusted. Colours end up in
 * a style attribute in the browser and are also used with an appended alpha
 * pair, so anything that is not a plain hex colour is replaced by the default.
 */
function ccrm_pdf_tpl_color($value, string $fallback): string {
    if (!is_string($value)) {
        return $fallback;
    }
    $v = trim($value);
...
/** Trim an untrusted string field to a sane column length. */
function ccrm_pdf_tpl_text($value, string $fallback, int $max = 255): string {
...
$badgeStyle = is_string($blueprint['badgeStyle'] ?? null) ? $blueprint['badgeStyle'] : 'rounded';
if (!in_array($badgeStyle, ['rounded', 'square', 'pill'], true)) {
    $badgeStyle = 'rounded';
}

$template = [
    'id' => 'act-' . bin2hex(random_bytes(6)),
    'name' => ccrm_pdf_tpl_text($blueprint['name'] ?? null, pathinfo($pdfName, PATHINFO_FILENAME) ?: 'AI Custom Template', 150),
    'description' => ccrm_pdf_tpl_text($blueprint['description'] ?? null, 'AI-analyzed custom price offer template', 500),
    'sourcePdfUrl' => $pdfUrl,
    'sourcePdfName' => $pdfName,
    'colors' => [
        'primary' => ccrm_pdf_tpl_color($blueprint['colors']['primary'] ?? null, '#0f172a'),
        'secondary' => ccrm_pdf_tpl_color($blueprint['colors']['secondary'] ?? null, '#334155'),
        'background' => ccrm_pdf_tpl_color($blueprint['colors']['background'] ?? null, '#ffffff'),
        'accent' => ccrm_pdf_tpl_color($blueprint['colors']['accent'] ?? null, '#f97316'),
        'text' => ccrm_pdf_tpl_color($blueprint['colors']['text'] ?? null, '#1e293b')
    ],
    'typography' => [
        'fontFamily' => ccrm_pdf_tpl_text($blueprint['typography']['fontFamily'] ?? null, 'Inter, Arial, sans-serif', 120),
        'headingStyle' => ccrm_pdf_tpl_text($blueprint['typography']['headingStyle'] ?? null, 'bold', 40)
    ],
    'sectionsOrder' => $sectionsOrder,
    'customBannerText' => ccrm_pdf_tpl_text($blueprint['customBannerText'] ?? null, 'Predbežná cena za komplexnú dodávku a montáž', 255),
