...
Owners: every value in the "Zodp." column must map to a real app user via
OWNER_MAP (or be listed in UNASSIGNED_OWNERS to import the lead unowned).
An unmapped code aborts the run - import_leads.php re-checks this against the
`users` table anyway, so a phantom owner cannot reach the database.
...
# Owner initials from the "Zodp." column mapped to the exact `users.name` of
# an existing app user. Nothing is ever passed through as literal sheet text:
# an unmapped code aborts the run (see resolve_owner), because leads owned by
# a person with no account show up all over the app - task boards, filters,
# dashboards - as a user that cannot be assigned, logged in as, or filtered out.
OWNER_MAP = {
    "C": "Cyprián",
    "T": "Tamara Majbová",   # written as "Tata" (nickname) in older sheets
    "TATA": "Tamara Majbová",
    "F": "Ildikó Fekete",
    "M": "Nora Moertl",
}

# Codes for people who never had an app account. Their leads are imported
# unowned rather than under an invented user name.
UNASSIGNED_OWNERS = {"K", "KATKA", "L"}

# Several rows share a lead between two people ("M/C", "C / F", "K/T"). The app
# stores a single owner, so the first initial that resolves to a real user wins.
...
def resolve_owner(v):
    """Sheet owner cell -> app user name, "" for unowned.

    Returns (owner, unknown_code). `unknown_code` is set when the cell holds a
    code this script has never seen; the caller collects those and aborts so a
    new colleague in the sheet is never silently imported as a phantom user.
    """
    s = clean(v)
    if not s:
        return "", None
    codes = [c for c in OWNER_SPLIT.split(s) if c]
    for c in codes:
        mapped = OWNER_MAP.get(c.upper())
        if mapped:
            return mapped, None
    # Nothing resolved. Known-but-accountless people import unowned; anything
    # else is a code we cannot interpret.
    if all(c.upper() in UNASSIGNED_OWNERS for c in codes):
        return "", None
    return "", s
...
def main():
    ...
    leads = []
    unknown_owners = {}  # sheet code -> [sheet row numbers]
    seq = 0
    sheet_row = 3  # iter_rows starts at 4; incremented at the top of the loop
    for row in ws.iter_rows(min_row=4, values_only=True):
        ...
        owner, unknown = resolve_owner(row[COL_OWNER])
        if unknown:
            unknown_owners.setdefault(unknown, []).append(sheet_row)
    ...
    if unknown_owners:
        print("ABORT: the 'Zodp.' column holds codes this script cannot map to", file=sys.stderr)
        print("an app user. Add them to OWNER_MAP (or UNASSIGNED_OWNERS if the", file=sys.stderr)
        print("person has no CRM account) and re-run:", file=sys.stderr)
        for code, rows in sorted(unknown_owners.items()):
            shown = ", ".join(str(r) for r in rows[:10])
            more = f" (+{len(rows) - 10} more)" if len(rows) > 10 else ""
            print(f"  {code!r}: {len(rows)} row(s) - sheet rows {shown}{more}", file=sys.stderr)
        sys.exit(1)
