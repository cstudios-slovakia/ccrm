...
// Fetch stored SuperFaktura configuration if not explicitly provided
...
$email = trim($data['email'] ?? ($sfConfig['email'] ?? ''));
$apiKey = trim($data['apiKey'] ?? ($sfConfig['apiKey'] ?? ''));
$companyId = trim((string)($data['companyId'] ?? ($sfConfig['companyId'] ?? '')));
$isSandbox = !empty($data['sandbox']) || !empty($sfConfig['sandbox']);
