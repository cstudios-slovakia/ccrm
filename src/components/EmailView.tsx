...
export const EmailView: React.FC<EmailViewProps> = ({
  ...
  const loadEmails = async (folder = activeFolder, currPage = page, currFilter = filter) => {
    ...
    try {
      const res = await fetch(`/api/mail_broker.php?action=get_emails&folder=${encodeURIComponent(folder)}&page=${currPage}&filter=${currFilter}`, {
        headers: { "X-User-Email": currentUser.email }
      });
      const data = await res.json();
      if (data.success) {
        ...
        notify(data.error || "Failed to fetch emails headers", "error");
      }
    } catch (err) {
  ...
  const expandThreadMessage = async (email: any) => {
    ...
    try {
      const res = await fetch(`/api/mail_broker.php?action=get_email_detail&uid=${email.uid}&folder=${encodeURIComponent(activeFolder)}`, {
        headers: { "X-User-Email": currentUser.email }
      });
      const data = await res.json();
      if (data.success) {
  ...
  const handleSendEmail = async (composer: any) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setComposers(prev => prev.filter(c => c.id !== composer.id));
        loadEmails(activeFolder, 1, filter);
      } else {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 select-none h-[calc(100vh-140px)] items-start overflow-hidden">
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-1.5">
                    <GeometricIcon emailString={email.from.address} />
      ...
      {composers.map(comp => (
        <div key={comp.id} className="fixed bottom-0 right-10 w-96 bg-white border-t border-x border-slate-300 rounded-t-2xl shadow-2xl z-50 flex flex-col">
          <div className="bg-slate-900 text-white p-3 rounded-t-2xl flex items-center justify-between">
            ...
            <button
              onClick={() => setComposers(prev => prev.filter(c => c.id !== comp.id))}
