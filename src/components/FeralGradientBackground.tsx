...
export const FeralGradientBackground: React.FC<FeralGradientBackgroundProps> = ({
  ...
  // Target shader uniforms based on current phase
  const targetTheme = LOGIN_PHASES[phaseId] || LOGIN_PHASES.day;
  const targetConfig = targetTheme.shaderConfig;

  // Refs to hold active interpolated colors for smooth morphing transitions
  const currentUniformsRef = useRef({
    u_main: [...targetConfig.u_main] as [number, number, number],
    u_low: [...targetConfig.u_low] as [number, number, number],
    u_mid: [...targetConfig.u_mid] as [number, number, number],
    u_high: [...targetConfig.u_high] as [number, number, number],
    u_wind: targetConfig.u_wind,
    u_warp: targetConfig.u_warp,
    u_nscale: targetConfig.u_nscale
  });
  ...
  useEffect(() => {
    ...
    // Geometry buffer (full viewport quad)
    ...
    // Lerp helper for smooth morphing transitions between atmospheric themes
  ...
  return (
    ...
    >
      {/* Animated WebGL Canvas with smooth color morphing */}
      <canvas
        ref={canvasRef}
        className="w-full h-full absolute inset-0 block object-cover"
        style={{ background: targetConfig.bg }}
      />

      {/* Fallback Static Gradient Mesh */}
      {!hasWebGL && (
        <div
          className="absolute inset-0 w-full h-full"
          style={{ background: targetTheme.backgroundGradient }}
        />
      )}
...
export const FeralSkyGradient = FeralGradientBackground;
