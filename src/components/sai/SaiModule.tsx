...
export const SaiModule: React.FC<SaiModuleProps> = ({ isDemoMode = false }) => {
  ...
  const [activeView, setActiveView] = useState<'list' | 'running' | 'report'>('list');
  ...
  // Launch New Simulation Rehearsal
  const handleLaunchSimulation = async (config: SimulationParameters) => {
    ...
    if (demoModeActive) {
      try {
        const demoSimId = `demo-sim-${Date.now()}`;
        ...
        <div className="flex items-center gap-2.5">
          ...
          {activeSimulationId && (
            <button
              ...
              className="px-3 py-2 rounded-2xl bg-white border border-slate-200 hover:border-purple-300 hover:bg-purple-50/50 text-slate-700 font-bold text-xs flex items-center gap-1.5 shadow-sm transition"
            >
          ...
          <button
            ...
            className="px-4 py-2 rounded-2xl bg-gradient-to-r from-purple-600 via-indigo-600 to-emerald-500 hover:from-purple-700 hover:to-emerald-600 text-white font-bold text-xs shadow-md hover:shadow-lg transition flex items-center gap-1.5"
          >
            ...
            <div className="p-8 rounded-3xl bg-gradient-to-br from-purple-900 via-indigo-950 to-slate-900 text-white shadow-xl relative overflow-hidden">
              <div className="relative z-10 max-w-2xl space-y-3">
                ...
                <div className="pt-2 flex items-center gap-3">
                  <button
                    onClick={() => setIsWizardOpen(true)}
                    className="px-5 py-2.5 rounded-2xl bg-gradient-to-r from-emerald-500 to-teal-400 hover:from-emerald-600 hover:to-teal-500 text-slate-950 font-extrabold text-xs shadow-lg transition flex items-center gap-2"
                  >
                    <Plus className="w-4 h-4" />
            ...
            <div className="space-y-4">
              ...
              {loadingList ? (
                ...
                <div className="p-12 rounded-3xl bg-white border border-dashed border-slate-300 text-center space-y-3">
                  ...
                  <button
                    onClick={() => setIsWizardOpen(true)}
                    className="px-4 py-2 rounded-xl bg-purple-600 text-white text-xs font-bold shadow hover:bg-purple-700 transition"
                  >
                    Create Simulation
                  </button>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {pastSimulations.map((sim) => (
                    <div
                      key={sim.id}
                      onClick={() => handleOpenPastSimulation(sim)}
                      className="p-5 rounded-3xl bg-white border border-slate-200/90 hover:border-purple-400 hover:shadow-lg transition cursor-pointer flex flex-col justify-between group"
                    >
                      <div className="space-y-3">
                        ...
                        <div>
                          <h4 className="text-sm font-bold text-slate-900 group-hover:text-purple-600 transition line-clamp-1">
                            {sim.title}
                          </h4>
                      ...
                      <div className="pt-4 mt-4 border-t border-slate-100 flex items-center justify-between text-xs font-bold text-purple-600 group-hover:text-purple-700">
                        <span>{sim.status === 'completed' ? 'View Executive Briefing' : 'Enter War Room'}</span>
