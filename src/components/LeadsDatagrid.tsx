...
const GATE_DEADLINE_TIME_PRESETS = [
    "09:00",
    "10:00",
    "12:00",
    "14:00",
...
const CalendarPane: React.FC<{
    ...
    const weekNumbers = useMemo(() => {
        return Array.from({ length: 5 }, (_, weekIdx) => {
            ...
            const weekNum =
                1 + Math.ceil((firstThursday - tempDate.valueOf()) / 604800000);
            return `W${weekNum}`;
        });
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            {isToday && (
                                                <span
                                                    className={`absolute bottom-0.5 h-1 w-1 rounded-full ${isStart || isEnd ? "bg-white" : "bg-purple-600"}`}
                                                />
                                            )}
                                        </button>
                                    </div>
...
const getPresetTranslationKey = (name: string) => {
    switch (name) {
        case "Today":
            return "dashboard.picker.today";
        case "Yesterday":
            return "dashboard.picker.yesterday";
        case "This week":
            return "dashboard.picker.this_week";
        case "Last week":
            return "dashboard.picker.last_week";
        case "This month":
            return "dashboard.picker.this_month";
        case "Last month":
            return "dashboard.picker.last_month";
        case "This quarter":
            return "dashboard.picker.this_quarter";
        case "Last quarter":
            return "dashboard.picker.last_quarter";
        case "This year":
            return "dashboard.picker.this_year";
        case "Last year":
            return "dashboard.picker.last_year";
        case "All Time":
            return "dashboard.picker.all_time";
        default:
            return "";
    }
};
...
const generateAiSummary = (
    ...
    if (lang === "sk") {
        ...
        if (statusLower === "rejected") {
            ...
            if (latestEventStr)
                desc += ` Posledná aktivita pred uzavretím: ${latestEventStr}.`;
            return desc;
        }
        ...
        if (statusLower === "contacted") {
            ...
            if (categoriesText)
                desc += ` a következő kategóriákban: ${categoriesText}`;
            if (latestEventStr)
                desc += `. Utolsó interakció: ${latestEventStr}`;
        ...
        if (statusLower === "offer sent") {
            ...
            if (latestEventStr)
                desc += ` Utolsó egyeztetés: ${latestEventStr}.`;
            return desc;
        }
        if (statusLower === "accepted") {
            let desc = `Sikeres üzlet. Az ajánlatot elfogadta az ügyfél`;
            if (lead.value > 0)
                desc += ` ${formatAmount(lead.value)} összértékben`;
        ...
        if (statusLower === "rejected") {
            ...
            if (latestEventStr)
                desc += ` Utolsó aktivitás a lezárás előtt: ${latestEventStr}.`;
            return desc;
        }
        ...
        if (statusLower === "offer sent") {
            ...
            if (lead.value > 0)
                desc += ` in the amount of ${formatAmount(lead.value)}`;
            if (categoriesText) desc += ` for ${categoriesText}`;
        ...
        if (statusLower === "accepted") {
            ...
            if (lead.value > 0)
                desc += ` for a total value of ${formatAmount(lead.value)}`;
        ...
        if (statusLower === "rejected") {
            ...
            if (latestEventStr)
                desc += ` Last activity before close: ${latestEventStr}.`;
            return desc;
        }
...
export const LeadsDatagrid: React.FC<LeadsDatagridProps> = ({
    ...
    const getSafePMColor = (pmName: string) => {
        ...
        const lowerKey = Object.keys(projectManagerColors).find(
    ...
    const getNextTaskForLead = (leadId: string) => {
        const leadTasks = tasks.filter(
            (t) => t.relatedLeadId === leadId && !isDoneState(t.status),
        );
    ...
    const getNextState = (currentStatus: string) => {
        ...
        const activeMain = leadStateParents[sName] || sName;
        const majorStates = leadStates
            .filter((s) => !leadStateParents[s.toLowerCase()])
            .map((s) => s.toLowerCase());
        const activeSubstates = leadStates
            .filter((s) => leadStateParents[s.toLowerCase()] === activeMain)
            .map((s) => s.toLowerCase());
        ...
        if (leadStateParents[sName]) {
            ...
            if (subIdx !== -1 && subIdx < activeSubstates.length - 1) {
                ...
                return (
                    leadStates.find((s) => s.toLowerCase() === nextSub) ||
                    nextSub
                );
            } else {
                ...
                if (mainIdx !== -1 && mainIdx < majorStates.length - 1) {
                    ...
                    return (
                        leadStates.find((s) => s.toLowerCase() === nextMain) ||
                        nextMain
                    );
                }
            }
    ...
    const [recordingState, setRecordingState] = useState<
        "idle" | "recording" | "paused" | "stopped" | "none"
    >("none");
    ...
    const [uploadedAudioFile, setUploadedAudioFile] = useState<string | null>(
        null,
    );
                        ...
                        if (typeof (window as any).showToast === "function") {
                            (window as any).showToast(
                                t(
                                    ...
                                    "Hlasový záznam bol úspešne uložený!",
                                    "A hangfelvétel sikeresen mentve!",
                                ),
                            );
                        }
                    } else {
                    ...
                    if (typeof (window as any).showToast === "function") {
                        (window as any).showToast(
                            t(
                                ...
                                "Nahranie zvuku na server zlyhalo: ",
                                "A hang feltöltése a szerverre sikertelen: ",
                            ) + err.message,
                            "error",
                        );
                    }
            ...
            if (typeof (window as any).showToast === "function") {
                (window as any).showToast(
                    t(
                        ...
                        "A mikrofonhoz való hozzáférés megtagadva: ",
                    ) + err.message,
                    "error",
                );
            }
        ...
        if (
            confirm(
                t(
                    ...
                    "Biztosan eltávolítja ezt a felvételt?",
                ),
            )
        ) {
            if (audioRef.current) {
    ...
    const handleTranscribeMeeting = async () => {
        ...
        const activeMeetingId =
            recordingMeetingId || `note_event_${Date.now()}`;
                ...
                if (data.transcription) {
                    ...
                    setNoteBlocks((prev) => [
                        ...prev,
                        {
                            id: `b-trans-${Date.now()}`,
                            type: "paragraph",
                            content: `<strong>${t("Transcription:", "Prepis:", "Átirat:")}</strong> ${data.transcription}`,
                        },
                    ]);
                ...
                if (typeof (window as any).showToast === "function") {
                    (window as any).showToast(
                        t(
                            ...
                            "AI prepis bol úspešne dokončený!",
                            "Az AI átírás sikeresen befejeződött!",
                        ),
                    );
                }
            } else {
    ...
    >({});
    const getGroupVisibleCount = (key: string) =>
        groupVisibleCount[key] ?? LEADS_PAGE_SIZE;
    ...
    const [filterOfferStartDate, setFilterOfferStartDate] =
        useState<Date | null>(null);
    const [filterOfferEndDate, setFilterOfferEndDate] = useState<Date | null>(
        null,
    );
    ...
    const isStateClosedLocal = useCallback(
        (stateName: string) => {
            ...
            const isClosed =
                ...
                (leadStageGroups &&
                    parent &&
                    leadStageGroups[parent.toLowerCase()] === "closed");
            return !!isClosed;
        },
    ...
    const handleConvertToProject = (type: ProjectType) => {
        ...
        (type.attributes || []).forEach((attr) => {
            ...
            if (
                attrNameLower.includes("email") ||
                attrIdLower.includes("email")
            ) {
                dynamicData[attr.id] = activeLead.email || "";
            } else if (
                attrNameLower.includes("phone") ||
                attrIdLower.includes("phone") ||
                attrNameLower.includes("tel") ||
                attrIdLower.includes("tel")
            ) {
                dynamicData[attr.id] = activeLead.phone || "";
            } else if (
                attrNameLower.includes("city") ||
                attrIdLower.includes("city")
            ) {
                dynamicData[attr.id] = activeLead.city || "";
            } else if (
                attrNameLower.includes("address") ||
                attrIdLower.includes("address") ||
                attrNameLower.includes("street") ||
                attrIdLower.includes("street")
            ) {
                dynamicData[attr.id] = activeLead.address?.street || "";
            } else if (
                attrNameLower.includes("company") ||
                attrIdLower.includes("company") ||
                attrNameLower.includes("firma") ||
                attrIdLower.includes("firma")
            ) {
        ...
        (window as any).showToast(
            t(
                "Converted successfully! Redirecting to projects...",
                "Konverzia úspešná! Presmerovanie na projekty...",
                "Konverzió sikeres! Átirányítás a projektekhez...",
            ),
        );
    ...
    const activeLeadFingerprint = useMemo(() => {
        if (!activeLead) return "";
        const tasksStr = activeLeadTasks
            .map((t) => `${t.id}-${t.status}-${t.title}-${t.deadline}`)
            .join("|");
        const timelineStr = (activeLead.timeline || [])
            .map(
                (e) =>
                    `${e.id}-${e.type}-${e.timestamp}-${e.content || ""}-${e.amount || 0}`,
            )
            .join("|");
    ...
    useEffect(() => {
        ...
        const generateSummary = async () => {
            ...
            try {
                ...
                const priceOffers = leadTimeline.filter(
                    (e) => e.type === "offer",
                );
                const otherData = {
    ...
    const userEmailSettings = useMemo(() => {
        try {
            if (currentUser && currentUser.metadata_json) {
                const metadata =
                    typeof currentUser.metadata_json === "string"
...
//
    ...
    const getEventColors = (type: string) => {
        switch (type) {
            ...
            case "appointment":
                return {
                    dotBg: "bg-purple-600 border-purple-700 text-white shadow-md shadow-purple-500/20",
            ...
            case "order":
                return {
                    dotBg: "bg-teal-600 border-teal-700 text-white shadow-md shadow-teal-500/20",
                    badgeBg: "bg-teal-50 text-teal-700 border-teal-200",
                };
            case "proforma_invoice":
                return {
                    dotBg: "bg-cyan-600 border-cyan-700 text-white shadow-md shadow-cyan-500/20",
                    badgeBg: "bg-cyan-50 text-cyan-700 border-cyan-200",
                };
            case "advance_receipt":
                return {
                    dotBg: "bg-sky-600 border-sky-700 text-white shadow-md shadow-sky-500/20",
    ...
    const renderEventIcon = (type: string) => {
        switch (type) {
            case "phone":
                return <Phone className="h-3.5 w-3.5 stroke-[2.5]" />;
            case "email":
                return <Mail className="h-3.5 w-3.5 stroke-[2.5]" />;
            case "note":
                return <FileText className="h-3.5 w-3.5 stroke-[2.5]" />;
            case "offer":
                return <Euro className="h-3.5 w-3.5 stroke-[2.5]" />;
            case "appointment":
                return <Calendar className="h-3.5 w-3.5 stroke-[2.5]" />;
            case "order":
                return <ClipboardList className="h-3.5 w-3.5 stroke-[2.5]" />;
            case "proforma_invoice":
                return <ReceiptText className="h-3.5 w-3.5 stroke-[2.5]" />;
            case "advance_receipt":
                return <Wallet className="h-3.5 w-3.5 stroke-[2.5]" />;
            case "invoice":
    ...
    const eventTypeLabel = (type: string) => {
        switch (type) {
            case "phone":
                return t("Call", "Hovor", "Hívás");
            case "email":
                return t("Email", "E-mail", "E-mail");
            case "note":
                return t("Note", "Poznámka", "Jegyzet");
            case "offer":
                return t("Offer", "Ponuka", "Ajánlat");
            case "appointment":
                return t("Meet", "Meet", "Találkozó");
            case "order":
                return t("Order", "Objednávka", "Megrendelés");
            case "proforma_invoice":
                return t("Proforma", "Zálohová faktúra", "Előlegszámla");
            case "advance_receipt":
                return t(
                    "Advance receipt",
                    "Doklad o preddavku",
    ...
    const documentEventTitle = (type: string) => {
        switch (type) {
            case "order":
                return t(
                    "Order filed",
                    "Objednávka zaevidovaná",
                    "Megrendelés rögzítve",
                );
            case "proforma_invoice":
                return t(
                    "Proforma invoice filed",
                    "Zálohová faktúra zaevidovaná",
                    "Előlegszámla rögzítve",
                );
            case "advance_receipt":
                return t(
                    "Advance payment receipt filed",
                    "Doklad o prijatom preddavku zaevidovaný",
                    "Előlegbizonylat rögzítve",
                );
            case "invoice":
                return t(
                    "Invoice filed",
            ...
            case "delivery_note":
                return t(
                    "Delivery note filed",
                    "Dodací list zaevidovaný",
                    "Szállítólevél rögzítve",
                );
            default:
                return t(
                    "Document filed",
                    "Dokument zaevidovaný",
                    "Dokumentum rögzítve",
                );
        }
    };
            ...
            return [
                {
                    ...
                    path:
                        event.filePath ||
                        `/uploads/${event.id}_${event.fileName}`,
                },
            ];
        }
        return [];
                ...
                {attachments.map((att, idx) => (
                    <div
                        ...
                        onClick={(e) => {
                            e.stopPropagation();
                            const path =
                                att.path || `/uploads/${event.id}_${att.name}`;
    ...
    const handleUpdateLeadProfile = (e: React.FormEvent) => {
        ...
        if (!leadName.trim() || !leadValue.trim()) {
            (window as any).showToast(
                t(
                    "Name and Value are strictly required!",
                    "Názov a hodnota sú povinné!",
                    "A név és az érték kötelező!",
                ),
            );
            return;
        }
        ...
        if (isNaN(valNum)) {
            (window as any).showToast(
                t(
                    ...
                    "Zadajte platnú číselnú hodnotu!",
                    "Adjon meg egy érvényes számértéket!",
                ),
            );
            return;
        }
        ...
        setLeads((prev) =>
            prev.map((l) => {
                if (l.id === activeLead.id) {
                    ...
                    const newCreatedAt = leadCreatedAt
                        ? leadCreatedAt.slice(0, 10)
    ...
    const handleAddLeadTimelineEvent = async (e: React.FormEvent) => {
        ...
        if (logType === "phone") {
            titleString = t(
                ...
                "Odchádzajúci hovor ukončený",
                "Kimenő hívás befejezve",
            );
            if (!contentString)
                contentString = t(
                    "Completed voice call with customer regarding updates.",
                    "Ukončený telefonát so zákazníkom ohľadom aktualizácií.",
            ...
            titleString = t(
                "Internal Note Added",
                "Pridaná interná poznámka",
                "Belső jegyzet hozzáadva",
            );
            // Note type is rich: saved as JSON stringified blocks
            const hasContent = noteBlocks.some(
                (b) => b.content.trim().length > 0,
            );
            if (!hasContent && !uploadedAudioFile) {
                (window as any).showToast(
                    t(
                        "Please write down some details or record audio for the note!",
                        "Napíšte nejaké podrobnosti alebo nahrajte zvuk pre poznámku!",
                        "Írjon be néhány részletet, vagy rögzítsen hangot a jegyzethez!",
                    ),
                );
                return;
            }
            ...
            if (!logTime.trim()) {
                (window as any).showToast(
                    t(
                        ...
                        "Válassza ki a találkozó időpontját!",
                    ),
                );
                return;
            }
            if (!contentString)
                contentString = `${t("Client appointment set for", "Stretnutie s klientom naplánované na", "Ügyféltalálkozó időpontja")} ${logTime.trim()}`;
        } else if (logType === "offer") {
            titleString = t(
                "Commercial Proposal Submitted",
                "Cenová ponuka odoslaná",
                "Kereskedelmi ajánlat benyújtva",
            );
            ...
            if (isNaN(amt) || amt <= 0) {
                (window as any).showToast(
                    t(
                        ...
                        "Suma ponuky musí byť kladné číslo!",
                        "Az ajánlat összegének pozitív számnak kell lennie!",
                    ),
                );
                return;
            }
            ...
            if (!contentString)
                contentString = `${t("Submitted commercial proposal of", "Odoslaná cenová ponuka vo výške", "Benyújtott kereskedelmi ajánlat összege")} ${money(amt)} ${t("to client.", "klientovi.", "az ügyfélnek.")}`;
            ...
            if (logDocumentFiles.length === 0) {
                (window as any).showToast(
                    t(
                        ...
                        "Priložte aspoň jeden dokument!",
                        "Csatoljon legalább egy dokumentumot!",
                    ),
                );
                return;
            }
        ...
        let finalFileName =
            logType === "offer" && logFileName ? logFileName : undefined;
        let finalFileSize =
            logType === "offer" && logFileSize ? logFileSize : undefined;
        ...
        if (isDocumentEventType(logType) && logDocumentFiles.length > 0) {
            ...
            try {
                ...
                for (let i = 0; i < logDocumentFiles.length; i++) {
                    ...
                    if (!uploadData.success) {
                        throw new Error(
                            `${file.name}: ${uploadData.error || t("Failed to upload file", "Nahranie súboru zlyhalo", "A fájl feltöltése sikertelen")}`,
                        );
                    }
        ...
        const newEvent: TimelineEvent = {
            ...
            fileType:
                logType === "offer" && finalFileName ? logFileType : undefined,
            audioFile:
                logType === "note" && uploadedAudioFile
                    ? uploadedAudioFile
                    : undefined,
            transcription:
            ...
            const autoPMTask: Task = {
                ...
                description:
                    logType === "note"
                        ? t(
                              "Meeting Note Added",
                              "Pridaná poznámka zo stretnutia",
                              "Találkozó jegyzet hozzáadva",
                          )
                        : contentString ||
                          `${t("Scheduled for", "Naplánované na", "Ütemezve")} ${logDate} ${logTimeOfEvent}`,
    ...
    const handleStartEditEvent = (event: TimelineEvent) => {
        ...
        if (event.type === "note" && text.trim().startsWith("[")) {
            try {
                ...
                /* keep raw content if it is not valid block JSON */
            }
        }
    ...
    const handleSaveEditEvent = (eventId: string) => {
        ...
        (window as any).showToast?.(
            t(
                "Event updated.",
                "Udalosť bola upravená.",
                "Az esemény frissítve.",
            ),
        );
    };
    ...
    const handleAddInlineLockingTask = (e: React.FormEvent) => {
        ...
        const newLockingTask: Task = {
            ...
            description:
                systemLanguage === "sk"
                    ? "Vytvorené v detaile záujemcu"
                    : systemLanguage === "hu"
                      ? "Érdeklődő részleteinél létrehozva"
                      : "Created from Lead detail drawer",
            status: taskStates[0] || "New",
            priority: "high",
        ...
        if (!inlineName.trim() || !inlineValue.trim()) {
            (window as any).showToast(
                t(
                    ...
                    "Polia nemôžu zostať prázdne!",
                    "A mezők nem maradhatnak üresen!",
                ),
            );
            return;
        }
    ...
    const handleCreateLead = (e: React.FormEvent) => {
        ...
        if (isNaN(valNum) || valNum < 0) {
            (window as any).showToast(
                t(
                    "Please enter a valid numeric lead value!",
                    "Zadajte platnú číselnú hodnotu leadu!",
                    "Adjon meg egy érvényes számszerű lead értéket!",
                ),
            );
            return;
        }
        ...
        return leads
            .filter((lead) => {
                ...
                const matchesState =
                    ...
                    leadStateParents[(lead.status || "").toLowerCase()] ===
                        selectedState.toLowerCase();
                const matchesSource =
                    selectedSource === "all" ||
                ...
                if (
                    offerPresetName !== "All Time" &&
                    (filterOfferStartDate || filterOfferEndDate)
                ) {
                    const timeline = lead.timeline || [];
                    const offerEvents = timeline.filter(
                        (e) => e.type === "offer" && e.timestamp,
                    );
                    if (offerEvents.length === 0) {
                        matchesOfferDate = false;
                    } else {
                        const latestOffer = offerEvents.reduce(
                            (latest, current) => {
                        ...
                        const startLimit = filterOfferStartDate
                            ? new Date(
                                  filterOfferStartDate.getFullYear(),
                                  filterOfferStartDate.getMonth(),
                                  filterOfferStartDate.getDate(),
                                  0,
                                  0,
                                  0,
                              )
                            : null;
                        const endLimit = filterOfferEndDate
                            ? new Date(
                                  filterOfferEndDate.getFullYear(),
                                  filterOfferEndDate.getMonth(),
                                  filterOfferEndDate.getDate(),
                                  23,
                                  59,
                                  59,
                                  999,
    ...
    const groupedLeads = useMemo(() => {
        ...
        if (orderingMode === "state") {
            ...
            return rawGrouped.filter((group) =>
                resolvedVisibleStates.includes(group.state.toLowerCase()),
            );
        } else {
    ...
    const orderedAllStates = useMemo(() => {
        ...
        leadStates.forEach((state) => {
            ...
            if (!leadStateParents[stateLower]) {
                list.push(state);
                const substates = leadStates.filter(
                    (s) => leadStateParents[s.toLowerCase()] === stateLower,
                );
                list.push(...substates);
            }
        });
                ...
                <div className="p-8 glass-panel rounded-[28px] border-2 border-red-400 bg-white shadow-glass text-center space-y-4">
                    ...
                    <p className="text-xs text-slate-600 font-semibold">
                        ...
                        {t(
                            ...
                            "' nem található az aktív adatbázisban.",
                        )}
                    </p>
                    <button
                        onClick={() => {
                    ...
                    >
                        {t(
                            ...
                            "Späť na register leadov",
                            "Vissza a leadek nyilvántartásába",
                        )}
                    </button>
                </div>
            );
                                        ...
                                        <div>
                                            ...
                                            {isEditingLead ? (
                                                ...
                                                <span className="text-[9px] font-extrabold uppercase tracking-wide text-emerald-700">
                                                    ...
                                                    {leadClientType ===
                                                        "partner" &&
                                                        `🤝 ${systemLanguage === "sk" ? "Obchodný partner" : systemLanguage === "hu" ? "Kereskedő partner" : "Dealer Partner"}`}
                                                    {leadClientType ===
                                                        "person" &&
                                                        `👤 ${systemLanguage === "sk" ? "Súkromná osoba" : systemLanguage === "hu" ? "Magánszemély" : "Private Person"}`}
                                                </span>
                                            )}
                                        </div>
                                    </div>
                                            ...
                                            <span className="font-extrabold text-slate-700 block">
                                                <span className="flex items-center gap-1">
                                                    ...
                                                    <span className="line-clamp-1">
                                                        {clientCardData.street
                                                            ? `${clientCardData.street}, `
                                                            : ""}
                                                        {clientCardData.city}
                                                        {clientCardData.postalCode
                                                            ? ` (${clientCardData.postalCode})`
                                                            : ""}
                                                    </span>
                                                </span>
                            ...
                            >
                                ...
                                <div className="space-y-2 border-t-2 border-slate-100 pt-3 text-left">
                                    ...
                                    {isEditingLead ? (
                                                            ...
                                                            onChange={(e) => {
                                                                if (
                                                                    ...
                                                                    setLeadSelectedCategories(
                                                                        [
                                                                            ...leadSelectedCategories,
                                                                            cat,
                                                                        ],
                                                                    );
                                                                } else {
                                                                    setLeadSelectedCategories(
                                                                        leadSelectedCategories.filter(
                                                                            (
                                                                                c,
                                                                            ) =>
                                                                                c !==
                                        ...
                                        <div className="flex flex-wrap gap-1.5 pt-1">
                                            {leadSelectedCategories.length >
                                            0 ? (
                                                leadSelectedCategories.map(
                                                    (cat) => {
                                                        const color =
                                ...
                                <div className="space-y-2 border-t-2 border-slate-100 pt-3 text-left">
                                    <label className="text-[9px] font-black text-slate-500 uppercase tracking-wider flex items-center gap-1.5">
                                        ...
                                        {t(
                                            "Client's Interest / Problem to Solve",
                                            "Záujem klienta / Problém na riešenie",
                                            "Ügyfél érdeklődése / Megoldandó probléma",
                                        )}
                                    </label>
                                    {isEditingLead ? (
                                        <textarea
                                ...
                                {tasks.filter(
                                    ...
                                    <div className="text-center py-6 text-slate-400 text-[10px] font-semibold">
                                        {systemLanguage === "sk"
                                            ...
                                            : systemLanguage === "hu"
                                              ? "Nincsenek kapcsolódó feladatok."
                            ...
                            >
                                ...
                                <div className="space-y-2">
                                    <input
                                        ...
                                        placeholder={
                                            systemLanguage === "sk"
                                                ? "napr. Podpísať preberací protokol"
                                                : systemLanguage === "hu"
                                                  ? "pl. Átadás-átvételi jegyzőkönyv"
                                                  : "e.g. Sign handover protocol"
                                        }
                                                    ...
                                                    <div className="md:col-span-2 space-y-2 animate-in slide-in-from-left duration-200">
                                                        ...
                                                        <label className="flex items-center gap-1.5 px-3 py-2.5 rounded-xl bg-amber-50 hover:bg-amber-100/80 text-amber-800 border-2 border-amber-300 transition-all cursor-pointer text-[10px] font-black uppercase shadow-sm select-none w-fit">
                                                            ...
                                                            <input
                                                                type="file"
                                                                multiple
                                                                accept=".pdf,.doc,.docx,.xls,.xlsx,.txt,.jpg,.jpeg,.png,.heic,.heif"
                                                                className="hidden"
                                                                onChange={(
                                                                    e,
                                                                ) => {
                                                        ...
                                                        <div className="flex flex-col border border-slate-200 rounded-xl bg-white p-3 min-h-[180px] max-h-[320px] overflow-y-auto outline-none text-xs cursor-text">
                                                            <BlockEditor
                                                                ...
                                                                onChange={(
                                                                    ...
                                                                    const serializeBlocksToPlainText =
                                                                        (
                                                                            blks: EditorBlock[],
                                                                        ): string => {
                                                                            return blks
                                                    ...
                                                    <textarea
                                                        ...
                                                        placeholder={
                                                            logType
                                                                ? getTranslation(
                                                                      systemLanguage,
                                                                      "logger.details_placeholder_log",
                                                                  )
                                                                : getTranslation(
                                                                      systemLanguage,
                                                                      "logger.details_placeholder_generic",
                                                                  )
                                                        }
                                                        value={logContent}
                                                        onChange={(e) =>
                                        ...
                                        {futureEvents.map((event) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    {/* Middle dot icon */}
                                                    <div
                                                        className={`h-9.5 w-9.5 rounded-xl flex items-center justify-center border-2 shrink-0 z-10 shadow-sm transition-all duration-200 group-hover:scale-115 ${colors.dotBg}`}
                                                    >
                                                        {renderEventIcon(
                                                            event.type,
                                                        )}
                                                            ...
                                                            >
                                                                ...
                                                                if (
                                                                    ...
                                                                    try {
                                                                        ...
                                                                        console.error(
                                                                            "Failed to parse JSON note blocks in future event",
                                                                            e,
                                                                        );
                                                                    }
                                                                }
                                                            ...
                                                            >
                                                                ...
                                                                if (
                                                                    ...
                                                                    try {
                                                                        ...
                                                                        console.error(
                                                                            "Failed to parse JSON note blocks",
                                                                            e,
                                                                        );
                                                                    }
                                                                }
            ...
            <div className="glass-panel px-6 py-4 rounded-[26px] border border-blue-50 bg-white/85 shadow-glass flex flex-col lg:flex-row lg:items-center justify-between gap-4 select-none">
                ...
                <div className="flex flex-wrap items-center gap-2">
                    {stateGroupedLeads.map((group) => {
                        ...
                        const isActive =
                            resolvedVisibleStates.includes(stateLower);
                        ...
                        {hasActiveFilters && (
                            <button
                                ...
                                title={t(
                                    ...
                                    "Az összes aktív szűrő törlése",
                                )}
                            >
                                <X className="h-3.5 w-3.5" />
                                {t("Clear", "Vymazať", "Törlés")}
                            </button>
                        )}
                    </div>
                                                        ...
                                                        <button
                                                            ...
                                                            onClick={() => {
                                                                const {
                                                                    ...
                                                                    preset.getRange();
                                                                setFilterOfferStartDate(
                                                                    start,
                                                                );
                                                                setFilterOfferEndDate(
                                                                    end,
                                                                );
                                                                setOfferPresetName(
                                                                    preset.name,
                        ...
                        <table className="w-full border-collapse text-left block lg:table">
                            <thead className="hidden lg:table-header-group">
                                <tr className="bg-white text-blue-600 text-[10px] font-black uppercase tracking-wider">
                                    ...
                                    <th className="sticky top-0 bg-white z-10 py-4 px-4 border-b-2 border-slate-100">
                                        {systemLanguage === "sk"
                                            ? "Dátum vytvorenia"
                                            : systemLanguage === "hu"
                                              ? "Létrehozás dátuma"
                                    ...
                                    <th className="sticky top-0 bg-white z-10 py-4 px-4 text-right border-b-2 border-slate-100">
                                        {getTranslation(
                                            systemLanguage,
                                            "leads.table.value",
                                        )}
                                    </th>
                                    <th className="sticky top-0 bg-white z-10 py-4 px-4 text-center border-b-2 border-slate-100">
                                        {getTranslation(
                                            systemLanguage,
                                            "leads.table.state",
                                        )}
                                    </th>
                                    <th className="sticky top-0 bg-white z-10 py-4 px-6 rounded-tr-[24px] text-center w-28 border-b-2 border-slate-100">
                                        {getTranslation(
                                            systemLanguage,
                                            "leads.table.actions",
                                        )}
                            ...
                            <tbody className="divide-y-0 lg:divide-y lg:divide-blue-50 text-xs block lg:table-row-group">
                                {groupedLeads.map((group) => {
                                    const stateColor =
                                        group.colorOverride ||
                                        getSafeStateColor(group.state);
                                    const stageTotalValue = group.leads.reduce(
                                        (acc, curr) => acc + curr.value,
                                        0,
                                    );
                                    const isCollapsed =
                                        !!collapsedGroups[group.state];
                                    const visibleCount = getGroupVisibleCount(
                                        group.state,
                                    );
                                    const hasMore =
                                        group.leads.length > visibleCount;
                                                        ...
                                                        <div className="flex items-center justify-between w-full gap-2">
                                                            <div className="flex items-center gap-2 shrink-0">
                                                                ...
                                                                >
                                                                    {orderingMode ===
                                                                    "pm"
                                                                        ? `${t("Project Manager", "Projektový manažér", "Projektmenedzser")}: ${group.state}`
                                                                        : group.state}
                                                                </span>
                                                                <span
                                                ...
                                                (group.leads.length === 0 ? (
                                                    <tr className="block lg:table-row">
                                                        ...
                                                        >
                                                            {t(
                                                                "No active leads in",
                                                                "Žiadne aktívne leady v",
                                                                "Nincs aktív lead itt:",
                                                            )}{" "}
                                                            {group.state}
                                                        </td>
                                                    </tr>
                                                                                    ...
                                                                                    <div className="flex items-center justify-center gap-2">
                                                                                        ...
                                                                                        <button
                                                                                            ...
                                                                                            title={t(
                                                                                                ...
                                                                                                "Soron belüli szerkesztés megszakítása",
                                                                                            )}
                                                                                        >
                                                                                            <X className="h-3.5 w-3.5" />
                                                                                        </button>
                                                                                                ...
                                                                                                segments.push(
                                                                                                    {
                                                                                                        ...
                                                                                                        tooltip:
                                                                                                            isLeadClosed
                                                                                                                ? `${t("Closed", "Uzavreté", "Lezárva")} (${lead.status})`
                                                                                                                : `${t("Closed", "Uzavreté", "Lezárva")} (${combinedTitle})`,
                                                                                                    },
                                                                                                );
                                                                                            }
                        ...
                        {stateGroupedLeads
                            .filter((group) =>
                                resolvedVisibleStates.includes(
                                    group.state.toLowerCase(),
                                ),
                            )
                            .map((group, index) => {
                                const stateColor = getSafeStateColor(
                                    group.state,
                                );
                                const totalVal = group.leads.reduce(
                                    (sum, l) => sum + l.value,
                                    0,
                                );
                                                                ...
                                                                return (
                                                                    ...
                                                                    >
                                                                        ...
                                                                        {(() => {
                                                                            ...
                                                                            return (
                                                                                <div
                                                                                    className="mt-2 pt-2 border-t border-slate-100/60 flex items-center gap-1.5 shrink-0"
                                                                                    onClick={(
                                                                                        e,
                                                                                    ) =>
                                                                                        e.stopPropagation()
                    ...
                    <div className="flex items-center gap-1.5">
                        ...
                        <span>
                            {t(
                                ...
                                "Kliknite na ľubovoľnú bunku pre priamu úpravu",
                                "Kattintson bármelyik cellára a soron belüli szerkesztéshez",
                            )}{" "}
                            &bull;{" "}
                            {t(
                                "Slideout opens via Edit button",
                                "Panel sa otvorí tlačidlom Upraviť",
                                "A panel a Szerkesztés gombbal nyílik meg",
                            )}
                        </span>
                    </div>
                    <div>
                            ...
                            <div className="flex flex-col">
                                ...
                                <p className="text-[10px] text-slate-400 uppercase font-semibold mt-0.5">
                                    {t(
                                        ...
                                        "Adjon új sorokat az aktív lead táblázathoz",
                                    )}{" "}
                                    {systemName}{" "}
                                    {t(
                                        "leads datagrid (Bottom Slideout)",
                                        "(spodné okno)",
                                        "(alsó panel)",
                                    )}
                                </p>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="space-y-1">
                                    ...
                                    <input
                                        ...
                                        value={newLeadValue}
                                        onChange={(e) =>
                                            setNewLeadValue(e.target.value)
                                        }
                                        placeholder={t(
                                            "e.g. 15000",
                                            "napr. 15000",
                  ...
                  into the fixed category checkboxes. */}
                            <div className="space-y-1 border-t border-slate-100 pt-3">
                                ...
                                <textarea
                                    ...
                                    onChange={(e) =>
                                        setNewLeadInterestNote(e.target.value)
                                    }
                                    placeholder={t(
                                        "What does the client need? What problem are we solving for them?",
                                        "Čo klient potrebuje? Aký problém mu riešime?",
                            ...
                            <div className="space-y-1">
                                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider flex items-center gap-1.5">
                                    ...
                                    {t(
                                        ...
                                        "Odporúčanie leadu (Odporúčané klientom)",
                ...
                >
                    ...
                    >
                        ...
                        <form
                            ...
                            className="flex-1 overflow-y-auto py-5 space-y-5 pr-1 scrollbar-thin"
                        >
                            {/* Name */}
                            <div className="space-y-1">
                                ...
                                <p className="text-[9px] text-slate-400">
                                    {t(
                                        ...
                                        "Az ügyfél nevének módosítása automatikusan újra hozzárendeli az összes erre az ügyfélre hivatkozó leadet.",
                                    )}
                                </p>
                            </div>
                            ...
                            <div className="space-y-2 border-t border-slate-100 pt-4">
                                <label className="text-[10px] font-extrabold text-emerald-600 uppercase tracking-wider flex items-center gap-1">
                                    ...
                                    {t(
                                        ...
                                        "Priradené aktívne leady",
                                        "Kapcsolódó aktív leadek",
                                    )}
                                </label>
                        ...
                        <div className="pt-4 border-t border-slate-100 flex gap-3">
                            ...
                            >
                                ...
                                {t(
                                    "Save Client Profile",
                                    "Uložiť profil klienta",
                                    "Ügyfélprofil mentése",
                                )}
                            </button>
                        </div>
                    </div>
